...
export default function CourseMatchPage() {
  ...
  try {
    ...
    const res = await fetch("http://localhost:5000/api/recommendations", {
      ...
      body: JSON.stringify({
        topic: values.description, // 👈 map description → topic
        level: "Beginner",
      }),
    });
    ...
    const formatted = {
      suggestions: data.data.map((item: any) => ({
        ...
        universityName: "Recommended Course",
        matchScore: Math.floor(Math.random() * 40) + 60, // fake score (60–100)
        rationale: "Recommended based on your interests.",
      })),
    };
